...
$("#submitCity").on("click", function (event) {
  ...
  function renderSearches() {
    ...
    for (let i = 0; i < searchHistory.length; i++) {
      let button = $("<button type='button' class='btn btn-primary'>").text(
        searchHistory[i]
      );
  ...
  $.ajax({
    ...
    let mainTempData = $("#mainTemp").text(
      "Temperature: " + JSON.parse(response.main.temp)
    );
    console.log(mainTempData);
    // displays main humidity
    let mainHumidityData = $("#mainHumidity").text(
      "Humidity: " + JSON.parse(response.main.humidity)
    );
    console.log(mainHumidityData);
    // displays main WindSpeed
    let mainWindSpeedData = $("#mainWindSpeed").text(
      "Wind Speed: " + JSON.parse(response.wind.speed)
    );
  ...
  $.ajax({
    ...
    let temp1Data = $("#temp1").text(
      "Temperature: " + JSON.parse(result.list[0].main.temp)
    );
    let humidity1Data = $("#humidity1").text(
      "Humidity " + JSON.parse(result.list[0].main.humidity)
    );

    let temp2Data = $("#temp2").text(
      "Temperature: " + JSON.parse(result.list[6].main.temp)
    );
    let humidity2Data = $("#humidity2").text(
      "Humidity " + JSON.parse(result.list[6].main.humidity)
    );

    let temp3Data = $("#temp3").text(
      "Temperature: " + JSON.parse(result.list[14].main.temp)
    );
    let humidity3Data = $("#humidity3").text(
      "Humidity: " + JSON.parse(result.list[14].main.humidity)
    );

    let temp4Data = $("#temp4").text(
      "Temperature: " + JSON.parse(result.list[22].main.temp)
    );
    let humidity4Data = $("#humidity4").text(
      "Humidity: " + JSON.parse(result.list[22].main.humidity)
    );

    let temp5Data = $("#temp5").text(
      "Temperature: " + JSON.parse(result.list[30].main.temp)
    );
    let humidity5Data = $("#humidity5").text(
      "Humidity: " + JSON.parse(result.list[30].main.humidity)
    );
